...
unsigned char obtenerFicha(const unsigned char* tablero,int indice)
{
    ...
    bloque = bloque | (static_cast<unsigned int>(tablero[byteInicial + 1]) << 8);
...
void escribirFicha(unsigned char* tablero,int indice,unsigned char valor)
{
    ...
    if (offset <= 5)
    {
        unsigned char mascara = static_cast<unsigned char>(0b00000111 << offset);

        // limpiamos los 3 bits donde irá la ficha.
        tablero[byteInicial] = tablero[byteInicial] & static_cast<unsigned char>(~mascara);

        // Luego escribimos el valor nuevo.
        tablero[byteInicial] = tablero[byteInicial] | static_cast<unsigned char>(valor << offset);
    ...
    unsigned int bloque = static_cast<unsigned int>(tablero[byteInicial]) | (static_cast<unsigned int>(tablero[byteInicial + 1]) << 8);

    unsigned int mascara = 0b00000111u << offset;
    ...
    bloque = bloque | (static_cast<unsigned int>(valor) << offset);

    // Se vuelven a guardar los dos bytes por separado.
    tablero[byteInicial] = static_cast<unsigned char>(bloque & 0xFF);

    tablero[byteInicial + 1] = static_cast<unsigned char>((bloque >> 8) & 0xFF);
}